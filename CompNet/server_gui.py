...
#----by Dr Y Gong---------------------------------------------------------#
#----This code is to be used in conjunction with client_gui.py------------#
#-------------------------------------------------------------------------#
from Tkinter import *
import socket
import threading
...
def pkg(Type, IPaddr, port, ID=""):
    ...
    if not ID:
        ...
        info = Type+':'+IPaddr + ':' + str(port) + ':' + str(ID)
    return info
...
class Server_listen_thread(threading.Thread):
    ...
    def run(self):
        while self.flag:
            conn,addr = s.accept()
            info = conn.recv(1024)
            # print "DK: info IN SERVER: %s" % info
            ...
            # with all clients address information saved in its client_list
                ...
                client_ID = dpkg(info)[3] # DK: client ID retrieved here
                # print "DK: client_ID IN SERVER: %s" % str(client_ID)
                ...
                if not client_list:
                    ...
                    # This is what needs changing to pass the ID name to the client.
                    for ADDR in client_list:
                        # print "DK: client_list.keys() IN SERVER: %s" % str(client_list)
                        # print "DK: ADDR (keys) IN SERVER: %s" % str(ADDR)
                        # print "DK: ADDR (values) IN SERVER: %s" % str(client_list[ADDR])
                        conn.send('-'+pkg('R', ADDR[0], ADDR[1], client_list[ADDR]))
